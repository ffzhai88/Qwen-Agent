...
class Agent(ABC):
    ...
    def __init__(self,
        ...
        Args:
            function_list: One list of tool name, tool configuration or Tool object,
              such as 'code_interpreter', {'name': 'code_interpreter', 'timeout': 10}, or CodeInterpreter(). ## 一个工具列表，列表元素可以是str 表示工具名称，或工具配置dictionary，或工具对象
            llm: The LLM model configuration or LLM model object.
              Set the configuration as {'model': '', 'api_key': '', 'model_server': ''}. ## 配置语言模型，可以是一个LLM（BaseChatModel实例），或者一个dictionary，存放多个LLM
            system_message: The specified system message for LLM chat.   ### 给LLM的初始message
            name: The name of this agent.
            description: The description of this agent, which will be used for multi_agent.
        """
        if isinstance(llm, dict):  ### 读入llm 
            self.llm = get_chat_model(llm)
        else:
            self.llm = llm

        self.function_map = {}  ### self.function_map存放所有的工具
        if function_list:
            for tool in function_list: ### 读入工具列表，如果tool是str或dictionary，则需要这个工具已经在TOOL_RIGISTRY中。function_map中的key为工具名称，value为BaseTOOl实例。
    ...
    def _run(self,
        ...
        """Return one response generator based on the received messages.  基于接受到的信息返回一个response generator
    ...
    def _call_llm(
        ...
        """The interface of calling LLM for the agent.  基于给定的messages，为agent调用LLM的接口函数，返回LLM的response generator
    ...
    def _call_tool(self,
                   ...
                   **kwargs) -> str:
        """The interface of calling tools for the agent. 调用对应的工具获得工具调用结果，如果结果是string则直接返回，如果不是则转换为JSON返回。
    ...
    def _init_tool(self, tool: Union[str, Dict, BaseTool]):  ###用于初始化工具列表中的每一个工具
    ...
    def _detect_tool(self, message: Message) -> Tuple[bool, str, str, str]:
        """A built-in tool call detection for func_call format message.  #### 检查LLM的输出是否要调用tool，并抽取调用tool相关的参数
